...
Далее засеять команды, роли и стартовые категории услуг:
...
```
Seeding EspoDental teams, roles, service categories...
  Teams: created 5 / total 5
  Roles: created 5 / total 5
  Service categories: created 8 / total 8
Done. Re-run is safe: the command is idempotent.
```

> Если на второй команде увидишь `Command 'espo-dental-seed-roles' is not
...
- [ ] `espo-dental-seed-roles` отчитался о созданных командах/ролях/
      категориях.
...
`espo-dental-seed-roles` **не нужно** — после первой ночной репликации
...
### `espo-dental-seed-roles` не найдено
...
sudo docker compose -f /volume1/docker/espodental/docker-compose.yml \
     exec espocrm php command.php espo-dental-seed-roles
```
